...
impl HandType {
    pub fn new(s: &str) -> HandType {
        ...
        let joker_value = card_value('J');

        if let Some(num_jokers) = card_count.get(&joker_value) {
            if *num_jokers == 5 {
            ...
            let highest_count = card_count
                .iter()
                .filter(|a| *a.0 != &joker_value)
                .max_by_key(|a| a.1)
                .unwrap();

            let card = *highest_count.0;
            card_count.insert(card, highest_count.1 + num_jokers);
            card_count.remove(&joker_value);
        }
